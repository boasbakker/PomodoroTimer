...
## Plan mode is seldom needed
Its the default. But I prefer just a few multiple choice checks and then you can code it. If I don't like it, we can always fix it later. If you are gonna write a plan, make it as concise as possible with only the possibly controversial changes.

## No bugs, good goding practises, good documentation, etc.
These things are always more important then adding new features. These should always be priority one, and before and after adding a new feature, some _laaghangend fruit_ should be fixed in terms of bugs / coding practises. This can be done without asking if it's obviously a bug/bad code. 
...
Pomodoro_timer/
├── README.md
├── CLAUDE.md                ← you are here (developer guide lives below)
